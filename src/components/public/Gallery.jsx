      ...
      <div className="max-w-site pad">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-12 mb-24">
          <div className="max-w-xl">
            <span className="text-forest-600 font-sans font-bold uppercase tracking-[0.3em] text-[10px]">Portfolio</span>
            <h2 className="h2 mt-8 text-forest-800">
              Capturing the <br />
              <span className="italic font-normal text-forest-600">art of illumination.</span>
            </h2>
          </div>
          <div>
            <div className="flex flex-wrap gap-6 border-b border-forest-800/10 pb-4">
              {CATS.map(({ v, l }) => (
                <button 
                  key={v} 
                  onClick={() => setActive(v)}
            ...
            {filtered.map((p, i) => (
              <div 
                ...
                className={`reveal d-${(i%5)*100}
                  ...
                  overflow-hidden bg-cream-50`}
              >

                {p.image_url
                  ? <img src={p.image_url} alt={p.title}
                         className="w-full h-auto grayscale-[50%] group-hover:grayscale-0 group-hover:scale-105 transition-all duration-1000"
                         loading="lazy" />
                ...
                {/* Hover overlay */}
                <div className="absolute inset-0 bg-forest-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex flex-col justify-end p-8">
                  <span className="text-[9px] font-bold uppercase tracking-widest text-white/60 mb-2">{p.category}</span>
                  <div className="text-white font-serif text-2xl">{p.title}</div>
                </div>
              </div>
            ))}
          </div>
